...
export default function Applications() {
  ...
  return (
    <div className="space-y-10" data-testid="applications-page">
      <header>
        <div className="text-[10px] uppercase tracking-[0.22em] text-emerald-700 font-bold">Pipeline</div>
                    ...
                    {items.map((a, i) => (
                      <motion.div key={a.application_id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <div className="mt-3 border-l-2 border-emerald-600 bg-slate-50 px-2.5 py-2">
                          <div className="text-[10px] uppercase tracking-wider font-bold text-emerald-700">Next step</div>
...
function Stat({ label, value, big, accent }) {
  const map = { amber: "text-amber-700", blue: "text-blue-700", emerald: "text-emerald-700", rose: "text-rose-600" };
  return (
    <div className={`p-5 border border-slate-200 ${big ? "bg-slate-900 text-white" : "bg-white"}`}>
      ...
      <div className={`mt-2 text-[11px] uppercase tracking-wider font-semibold ${big ? "text-emerald-300" : "text-slate-500"}`}>{label}</div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm" data-testid="application-panel">
      <div className="absolute right-0 top-0 h-full w-full max-w-xl bg-white shadow-2xl overflow-y-auto">
        <div className="sticky top-0 z-10 border-b border-slate-200 bg-white px-6 py-4 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase tracking-[0.2em] text-emerald-700 font-bold">Application</div>
            ...
            <div className="space-y-3">
              {(app.timeline || []).length === 0 ? (
                ...
                <div key={`${item.stage}-${item.date}-${index}`} className="flex gap-3">
                  <span className="mt-1.5 h-2 w-2 rounded-full bg-emerald-700 shrink-0" />
                  <div>
