...
export default function Dashboard() {
  ...
  if (loading) return <div className="flex items-center justify-center py-32"><Loader2 className="animate-spin text-emerald-600" /></div>;
  ...
  return (
    <div className="space-y-12 max-w-7xl" data-testid="dashboard-page">
      <header className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <div className="text-[10px] uppercase tracking-[0.22em] text-emerald-600 font-bold">Overview</div>
          <h1 className="mt-2 font-display text-5xl font-bold tracking-tight leading-none">
            Good morning, <span className="font-serif-display text-emerald-600">{(profile?.startup_name || "founder").split(" ")[0].toLowerCase()}.</span>
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {upcoming.slice(0, 3).map((d, i) => (
              <motion.div key={d.opportunity_id} variants={fade} initial="initial" animate="animate" transition={{ duration: 0.4, delay: i * 0.08 }}
                className="bg-emerald-50/70 border border-emerald-200 hover:border-emerald-300 p-5 transition-colors">
                <span className="inline-block text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">{d.deadline}</span>
                <div className="mt-4 font-display text-lg font-semibold text-slate-900 leading-tight">{d.title}</div>
                <div className="mt-3 text-emerald-700 text-sm font-medium">{d.org}</div>
              </motion.div>
            ))}
          </div>
        )}
      ...
      <section data-testid="drafts-section">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link to="/drafts" className="text-sm text-emerald-600 hover:text-emerald-900 font-medium" data-testid="view-all-drafts">View All Drafts -&gt;</Link>
        </div>
        {drafts.length === 0 ? (
          ...
          <div className="bg-white border border-slate-200 overflow-hidden">
            ...
            <div className="p-6">
              ...
              <Link to="/drafts">
                <Button className="mt-6 w-full h-12 rounded-md bg-emerald-700 hover:bg-emerald-800 text-white" data-testid="continue-draft-btn">
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          ...
          {matches.length === 0 ? (
            ...
            <div key={row.opportunity_id} className="grid grid-cols-12 px-6 py-4 items-center text-sm hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
              ...
              <div className="col-span-2 text-emerald-700 font-medium">{row.deadline}</div>
...
function KpiCard({ value, label, suffix = "", featured }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
      className={`p-6 border transition-all ${featured ? "bg-emerald-700 text-white border-emerald-700" : "bg-white border-slate-200 hover:border-slate-300"}`}>
