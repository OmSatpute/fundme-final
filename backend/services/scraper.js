...
function extractDeadline(text) {
  ...
  // 0. Explicit check for closed/expired status
  if (/closed|expired|applications\s*closed|notify\s*me\s*when\s*it\s*reopens/i.test(text)) {
    return 'Closed';
  }
  
  // 1. Prioritize relative days (e.g., "4d left", "Closing in 5d", "8 days remaining")
  const relativeMatch = text.match(/(?:closing\s+in\s+)?(\d+)\s*([dw])\s*(?:left|remaining|to\s+go)?/i) || 
                        text.match(/(\d+)\s*days?\s*remaining/i);
  
  if (relativeMatch) {
    const num = parseInt(relativeMatch[1], 10);
    const unit = (relativeMatch[2] || 'd').toLowerCase();
    const deadlineDate = new Date();
    if (unit === 'd' || relativeMatch[0].includes('day')) deadlineDate.setDate(deadlineDate.getDate() + num);
    else if (unit === 'w') deadlineDate.setDate(deadlineDate.getDate() + num * 7);
    return deadlineDate.toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' });
  }

  // 2. Handle "Closes today" or "Closes in X hours"
  if (/closes\s*today/i.test(text) || /\d+h\s+left/i.test(text)) {
    return new Date().toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' });
  }

  // 3. Handle standard date formats (e.g., "May 31", "31 May", "31/05/2026", "2026-05-31")
